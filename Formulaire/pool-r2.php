...
$subject = 'Nouvelle prédiction - Ronde 2 du pool';

$body  = "Nouvelle prédiction reçue - RONDE 2\n\n";
...
foreach ($RECIPIENTS as $addr => $name) {
  ...
  if ($addr !== '' && filter_var($addr, FILTER_VALIDATE_EMAIL)) {
    ...
    error_log("Destinataire ignoré : {$addr}");
  }
}
...
@file_put_contents(__DIR__ . '/pool-r2-submissions.csv', implode(';', $csv_line) . PHP_EOL, FILE_APPEND);

if ($ok) {
  echo json_encode(['ok' => true]);
} else {
  error_log('mail() a renvoyé false dans pool-r2.php');
